...
parser.add_argument('-tsample', type=int, default=3000000)     #采样次数
parser.add_argument('-w', type=float, default=[1,1,1])         #分别为β-dtau, β, β+dtau处的权重,为了之后算能量时进行微分计算使用
class WLQMC:
    def __init__(self, n, Jx, Jy, Jz, fieldh, tau, dtau, ntimes, tsample, heatn, w):
        self.n       = n
        self.m       = ntimes*2
        self.Jx      = Jx
        self.Jy      = Jy
    ...
    def local_update(self, nt): #一次local update(i,j为没有gate的plaquette的坐标,权重及其变化由其四周的plaquette贡献)
        ...
        # if (self.w[0]/self.w[2])==0:
        # with open(file_path, 'a') as f:
        #     print(self.w, file=f)
        # self.sumE   += (np.log(self.w[0]/self.w[2])/(self.m*self.dtau))*self.w[1]
        # self.sumw   += self.w[1]
        if nt>=self.heatn:  #热化完成再计数
            self.sumE   += (np.log(self.w[0]/self.w[2])/(self.m*self.dtau))
        ...
        self.w_ave  += self.w[1]
        self.w2_ave += self.w[1]**2
        if nt%1000==999: #每1000步记录一次
            self.lE.append(self.E_ave/1000)
            self.lw.append(self.w_ave/1000)
            self.lwerr.append(np.sqrt((self.w2_ave/999)-(self.w_ave/999)*(self.w_ave/1000)))
            self.E_ave = 0
            self.w_ave = 0
            self.w2_ave= 0
    ...
    def run_MC(self):
        ...
        self.E_err= np.sqrt(self.sumE2/(self.sumw-1) - (self.sumE/self.sumw)*(self.sumE/(self.sumw-1)))
    ...
    def check_heating(self):
        x = np.linspace(1000, 1000*len(self.lE), len(self.lE))
...
if __name__ =="__main__":
    ...
    with open(file_path, 'w') as f:
        pass
    MC_solver = WLQMC(args.n, args.Jx, args.Jy, args.Jz, args.fieldh, args.tau, args.dtau, args.ntimes, args.tsample, args.heatn, args.w)
